...
* **Statistical Modeling:** Linear Mixed-Effects Models (LMMs, via **lme4/emmeans**), Bayesian Analysis, Psychometric Function Fitting, **Drift-Diffusion Modeling (DDM)** with integration of physiological measures (pupillometry), **Linear Ballistic Accumulator (LBA)** modeling for cognitive decomposition of response time distributions (hierarchical Bayesian implementation via PyMC).
...
* **Extended Reality (XR) Platforms:** Building research-grade XR experimental platforms with adaptive modality systems, dual input methods (gaze and hand-tracking), real-time performance monitoring, and comprehensive data logging for cognitive load studies. Includes **physiologically-accurate gaze simulation** (fixation-state Gaussian jitter, saccadic suppression, confirmation model) for remote testing without eye-tracking hardware.
