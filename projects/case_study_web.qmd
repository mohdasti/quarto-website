...
if (is.null(src_file)) {
  # If file doesn't exist, skip this figure gracefully without showing warning
...
if (is.null(results_file)) {
  # Results CSV not found - create empty results to prevent errors
  # (Table will be skipped automatically if empty)
  results <- tibble(
    metric = character(0),
    Hand = character(0),
    Hand_N = character(0),
...
```{r endpoint-heatmap-static, eval=FALSE}
...
# Note: This plot requires data file - uncomment eval condition when data is available
# eval=has_highcharter && exists("df_endpoint_int") && !is.null(df_endpoint_int)
if (FALSE && has_highcharter && !is.null(df_endpoint_int) && !is.null(endpoint_axis_info)) {
  ...
  if (nrow(df_endpoint_static_data) > 0) {
    ...
    # No endpoint data available for static condition
    cat("\n\n*Endpoint density heatmap requires data file.*\n\n")
  }
} else {
...
```{r endpoint-heatmap-adaptive, eval=FALSE}
...
if (FALSE && has_highcharter && !is.null(df_endpoint_int) && !is.null(endpoint_axis_info)) {
  ...
  if (nrow(df_endpoint_both) > 0) {
    ...
    if (nrow(df_density_adaptive) > 0) {
      ...
      # No endpoint data available for adaptive condition
      cat("\n\n*Endpoint density heatmap (Adaptive) requires data file.*\n\n")
    }
  } else {
    # No endpoint data available
    cat("\n\n*Endpoint density heatmap (Adaptive) requires data file.*\n\n")
...
```{r spatial-heatmap-hand, eval=FALSE}
#| fig-cap: "Spatial Error Rate: Hand modality (interactive - hover for exact error rates, zoom/pan enabled)"
# Note: This plot requires data file - uncomment eval condition when data is available
# eval=has_highcharter && exists("df_spatial_int") && !is.null(df_spatial_int)
if (FALSE && has_highcharter && !is.null(df_spatial_int) && nrow(df_spatial_int) > 0) {
  ...
  if (nrow(df_hand_spatial) > 0) {
    ...
    # No hand spatial data available
    cat("\n\n*Spatial error heatmap for Hand modality requires data file.*\n\n")
  }
} else {
...
```{r spatial-heatmap-gaze, eval=FALSE}
...
if (FALSE && has_highcharter && !is.null(df_spatial_int) && nrow(df_spatial_int) > 0) {
  ...
  if (nrow(df_gaze_spatial) > 0) {
    ...
    # No gaze spatial data available
    cat("\n\n*Spatial error heatmap for Gaze modality requires data file.*\n\n")
  }
} else {
  # No spatial data available
  cat("\n\n*Spatial error heatmap for Gaze modality requires data file.*\n\n")
