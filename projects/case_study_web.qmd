...
date: "2026-02-01"
description: "I built a remote XR interaction testbed and evaluated whether a context-aware adaptive system can reduce gaze failure modes (e.g., Midas Touch slips: accidental activations) and workload under time pressure."
outcome: "Remote XR testbed · Bayesian LBA · arXiv preprint"
categories: ["XR/HCI", "Gaze Interaction", "Bayesian LBA", "Remote Study", "arXiv"]
...
<div class="project-snapshot-card">
  <div class="ps-item"><span class="ps-label">Role</span><span class="ps-value">Sole Researcher — Study Design, Development, Analysis</span></div>
  <div class="ps-item"><span class="ps-label">Methods</span><span class="ps-value">ISO 9241-9, Fitts' Law, NASA-TLX, Bayesian LBA (PyMC), Williams Counterbalancing</span></div>
  <div class="ps-item"><span class="ps-label">Stack</span><span class="ps-value">React, TypeScript, Python, PyMC, Quarto</span></div>
  <div class="ps-item"><span class="ps-label">Output</span><span class="ps-value">arXiv preprint · Remote testbed (live) · Case study</span></div>
</div>
```
...
body.quarto-dark .tip-i{ color:rgba(255,255,255,.75); background:rgba(255,255,255,.1); border-color:rgba(255,255,255,.25); }
body.quarto-dark .tip-i:hover{ background:rgba(109,40,217,.2); border-color:#a78bfa; }
...
<style>
/* Cards + chips */
.tldr-card{background:var(--color-bg-card,#ffffff);border:1px solid var(--color-border,#e5e7eb);border-left:4px solid #0ea5e9;border-radius:12px;padding:12px 14px;margin:8px 0 14px;box-shadow:0 1px 1px rgba(16,24,40,.02)}
.tldr-title{font-weight:700;letter-spacing:.2px;margin-bottom:.25rem}
.tldr-text{margin:.15rem 0 .6rem;color:var(--color-text,#111827)}
.tldr-body{margin:.15rem 0 .6rem;color:var(--color-text,#111827);line-height:1.5}
.tldr-chips{display:flex;flex-wrap:wrap;gap:6px}
.tldr-chips a{display:inline-flex;align-items:center;gap:.35rem;font-size:.86rem;padding:.18rem .5rem;border-radius:999px;border:1px solid var(--color-border,#d7dee6);background:var(--color-bg-subtle,#f8fafc);text-decoration:none;color:var(--color-text,#0b1526)}
.tldr-chips a:hover{background:#eef6fb;border-color:#b7e1f0}
body.quarto-dark .tldr-chips a:hover{background:rgba(14,165,233,.15);border-color:#0ea5e9}

.build-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;margin:.4rem 0 1rem}
.build-card{background:var(--color-bg-card,#ffffff);border:1px solid var(--color-border,#e5e7eb);border-radius:12px;padding:12px 14px;box-shadow:0 1px 1px rgba(16,24,40,.02)}
...
body.quarto-dark .build-icon{background:rgba(3,105,161,.15);border-color:rgba(3,105,161,.3);color:#38bdf8}
.build-title{font-weight:700;margin:0}
.build-sub{color:var(--color-text-muted,#4b5563);margin:.1rem 0 0;font-size:.92rem}
.build-list{margin:.55rem 0 0;padding-left:1.05rem}
.build-list li{margin:.2rem 0}
.build-kicker{font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--color-text-muted,#6b7280);margin-bottom:.4rem}
.build-card h3{font-weight:700;font-size:1.05rem;margin:.2rem 0 .4rem;color:var(--color-text,#111827)}
.muted{color:var(--color-text-muted,#6b7280);font-size:.92rem;line-height:1.5;margin:.3rem 0}

.help-icon{display:inline-flex;align-items:center;justify-content:center;width:1.05em;height:1.05em;border-radius:999px;border:1px solid var(--color-border,#e5e7eb);color:#0ea5e9;font-size:.8em;margin-left:.25rem;cursor:help;vertical-align:baseline}
.help-icon:hover{background:#e6f6ff;border-color:#bfe9ff}
body.quarto-dark .help-icon:hover{background:rgba(14,165,233,.15);border-color:#0ea5e9}
.tt-wide .tooltip-inner{max-width:360px;text-align:left;line-height:1.25;padding:.45rem .55rem}
.small-muted{color:var(--color-text-muted,#6b7280);font-size:.92rem}

/* Minimalist underline expandable section */
...
body.quarto-dark details.tech-details summary:hover,
body.quarto-dark details.tech-details[open] summary::after{
  color:#a78bfa;
}
...
if (!is.null(df_raw_interactive)) {
  ...
  # Fallback: try to get participant counts from results_at_a_glance.csv
  n_hand <- NA
  n_gaze <- NA
  n_total <- NA
  
  results_paths <- c(
    file.path(getwd(), "assets", "case_study", "results_at_a_glance.csv"),
    file.path(getwd(), "outputs", "results_at_a_glance.csv")
  )
  ...
  for (results_file in results_paths) {
    if (file.exists(results_file)) {
      tryCatch({
        ...
        # NASA-TLX row contains participant counts (not trial counts)
        if ("metric" %in% names(results) && "Hand_N" %in% names(results) && "Gaze_N" %in% names(results)) {
          tlx_row <- results %>% filter(grepl("NASA-TLX|TLX", metric, ignore.case = TRUE))
          if (nrow(tlx_row) > 0) {
            n_hand <- as.numeric(tlx_row$Hand_N[1])
            n_gaze <- as.numeric(tlx_row$Gaze_N[1])
            if (!is.na(n_hand) && !is.na(n_gaze)) {
              n_total <- max(n_hand, n_gaze, na.rm = TRUE)  # Use max since some participants may be in both
            }
            break
          }
        }
  ...
  # Initialize these to NULL if data doesn't load
  df_err_type_int <- NULL
  df_spatial_int <- NULL
  df_endpoint_int <- NULL
  endpoint_axis_info <- NULL
...
body.quarto-dark{
  --ink:#e8eaf0; --muted:#9ca3af; --bg:#0f1117; --card:#1e2130;
  --border:#2d3148; --soft:#181b23;
}
a{ color:var(--accent); }
...
/* --- Hero / TL;DR card --- */
.tldr-card{
  background:var(--card,#ffffff);border:1px solid var(--border,#e5e7eb);border-left:4px solid var(--accent);
...
.tldr-text{margin:.15rem 0 .6rem;color:var(--ink,#111827)}
.tldr-kpis{display:flex;flex-wrap:wrap;gap:10px;margin:.35rem 0 .55rem}
.kpi{background:var(--soft,#f8fafc);border:1px solid var(--border,#d7dee6);border-radius:12px;padding:8px 10px;min-width:170px}
.kpi .lbl{font-size:.78rem;color:var(--muted,#4b5563);margin-bottom:2px}
...
.tldr-chips a{
  ...
  padding:.18rem .5rem;border-radius:999px;border:1px solid var(--border,#d7dee6);
  background:var(--soft,#f8fafc);text-decoration:none;color:var(--ink,#0b1526)
}
.tldr-chips a:hover{background:#f3f0ff;border-color:#d9d0ff}
body.quarto-dark .tldr-chips a:hover{background:rgba(109,40,217,.2);border-color:#a78bfa}
...
.tldr-actions a.btn{
  ...
  padding:.32rem .68rem;border-radius:10px;border:1px solid var(--border,#d7dee6);
  background:var(--card,#ffffff);color:var(--ink,#0b1526);text-decoration:none
}
.tldr-actions a.btn.primary{background:#f3e8ff;border-color:#e9d5ff}
body.quarto-dark .tldr-actions a.btn.primary{background:rgba(109,40,217,.15);border-color:rgba(109,40,217,.35)}
...
.repo-card{
  background:var(--card,#fff);border:1px solid var(--border,#e5e7eb);border-radius:12px;padding:14px 14px 12px;
...
body.quarto-dark .repo-card:hover{border-color:rgba(109,40,217,.35)}
...
.repo-desc{color:var(--muted,#4b5563);margin:.35rem 0 .5rem;font-size:.95rem}
...
.repo-badges img{height:18px;border-radius:6px;border:1px solid var(--border,#edf0f3);display:block}
...
.repo-actions a.btn{
  ...
  border:1px solid var(--border,#d7dee6);background:var(--soft,#f8fafc);color:var(--ink,#0b1526);text-decoration:none
}
.repo-actions a.btn.primary{background:#f3e8ff;border-color:#e9d5ff}
body.quarto-dark .repo-actions a.btn.primary{background:rgba(109,40,217,.15);border-color:rgba(109,40,217,.35)}
  ...
  <div class="repo-card" role="group" aria-label="XR Adaptive Modality repository">
    ...
    <div class="repo-bar">
      <div class="repo-badges">
        ...
        <a href="https://arxiv.org/abs/2603.15991" target="_blank" rel="noopener">
          <img alt="arXiv" loading="lazy"
               src="https://img.shields.io/badge/arXiv-2603.15991-b31b1b.svg?logo=arxiv&logoColor=white">
        </a>
...
<div class="tldr-card" role="note" aria-label="What I built">
  <div class="tldr-title">What I shipped</div>
  <div class="tldr-body">
...
# Copy image to a location that will be accessible from the rendered HTML
# The HTML is rendered to docs/projects/case_study_web.html, so images should be in docs/projects/images/
output_dir <- knitr::opts_knit$get("output.dir")
if (is.null(output_dir)) {
  # Fallback: assume we're rendering to docs/projects/
  output_dir <- file.path(getwd(), "docs", "projects")
}
...
src_file <- NULL
possible_sources <- c(
  file.path(getwd(), "assets", "case_study", "portfolio", "architecture.png"),
  file.path(getwd(), "assets", "case_study", "architecture.png"),
  file.path(getwd(), "images", "architecture.png")
)
...
if (is.null(src_file)) {
  stop("Architecture image not found. Checked:\n", paste("  -", possible_sources, collapse = "\n"))
}
...
src_file <- NULL
possible_sources <- c(
  file.path(getwd(), "assets", "case_study", "participant_journey_final.png"),
  file.path(getwd(), "assets", "case_study", "portfolio", "participant_journey_final.png")
)
for (src in possible_sources) {
  if (file.exists(src)) {
    src_file <- src
...
if (is.null(src_file)) {
  stop("Participant journey image not found. Checked:\n", paste("  -", possible_sources, collapse = "\n"))
}
...
src_file <- NULL
possible_sources <- c(
  file.path(getwd(), "assets", "case_study", "Task_layout.png"),
  file.path(getwd(), "assets", "case_study", "portfolio", "Task_layout.png")
)
for (src in possible_sources) {
  if (file.exists(src)) {
    src_file <- src
...
if (is.null(src_file)) {
  # If file doesn't exist, skip this figure gracefully
  cat("\n\n⚠ **Note:** Task layout image not found. Skipping figure.\n")
  cat("Checked locations:\n")
  for (src in possible_sources) {
    cat("  -", src, ifelse(file.exists(src), "✓", "✗"), "\n")
  }
  cat("\n")
} else {
...
```{r results-dashboard-interactive}
#| fig-cap: "Results Dashboard: Comparison of key metrics by modality"
# Check if we have the required data for interactive plot
has_interactive_data <- FALSE
if (has_highcharter && exists("df_raw_interactive") && !is.null(df_raw_interactive)) {
  if (exists("df_pid_cond_int") && !is.null(df_pid_cond_int) && nrow(df_pid_cond_int) > 0) {
    has_interactive_data <- TRUE
  }
}

# Always show something - either interactive plot or fallback image
if (has_interactive_data) {
  ...
  hc_dashboard
    ...
    # Fallback to static image - always show this if interactive data is not available
    static_img <- file.path(getwd(), "assets", "case_study", "portfolio", "results_dashboard.png")
    if (file.exists(static_img)) {
      output_dir <- knitr::opts_knit$get("output.dir")
      if (is.null(output_dir)) {
        output_dir <- file.path(getwd(), "docs", "projects")
      }
      images_dir <- file.path(output_dir, "images")
      if (!dir.exists(images_dir)) dir.create(images_dir, recursive = TRUE)
      dst_file <- file.path(images_dir, "results_dashboard.png")
      if (!file.exists(dst_file) || file.mtime(static_img) > file.mtime(dst_file)) {
        file.copy(static_img, dst_file, overwrite = TRUE)
      }
      # Change to output directory so relative path works (same pattern as other images)
      old_wd <- getwd()
      on.exit(setwd(old_wd), add = TRUE)
      setwd(output_dir)
      knitr::include_graphics("images/results_dashboard.png", dpi = 200)
    } else {
      cat("\n⚠ **Note:** Results dashboard image not found at:", static_img, "\n")
    }
  }
```
...
```{r results-table}
# Try multiple possible source locations for results CSV
results_file <- NULL
possible_csv_sources <- c(
    file.path(getwd(), "assets", "case_study", "results_at_a_glance.csv"),
  file.path(getwd(), "outputs", "results_at_a_glance.csv")
)
for (src in possible_csv_sources) {
  if (file.exists(src)) {
    results_file <- src
    break
  }
...
if (is.null(results_file)) {
  cat("\n\n⚠ **Note:** Results CSV file not found. Skipping results table.\n")
  cat("Checked locations:\n")
  for (src in possible_csv_sources) {
    cat("  -", src, ifelse(file.exists(src), "✓", "✗"), "\n")
  }
  cat("\n")
  # Create empty results to prevent errors downstream
  results <- tibble(
    metric = character(0),
    Hand = character(0),
    Hand_N = character(0),
...
```{r error-type-table}
if (file.exists("assets/case_study/error_types_by_modality.csv")) {
  err_types <- read_csv("assets/case_study/error_types_by_modality.csv", show_col_types = FALSE)
...
```{r error-type-fig-interactive}
#| fig-cap: "Error Type Composition by Modality"
# Check if we have the required data for interactive plot
has_err_type_data <- FALSE
if (has_highcharter && exists("df_err_type_int") && !is.null(df_err_type_int)) {
  if (nrow(df_err_type_int) > 0) {
    has_err_type_data <- TRUE
  }
}

if (has_err_type_data) {
  ...
  hc_err_type
    ...
    # Fallback to static image - always show this if interactive data is not available
    static_img <- file.path(getwd(), "assets", "case_study", "error_type_composition.png")
    if (file.exists(static_img)) {
      output_dir <- knitr::opts_knit$get("output.dir")
      if (is.null(output_dir)) {
        output_dir <- file.path(getwd(), "docs", "projects")
      ...
      images_dir <- file.path(output_dir, "images")
      if (!dir.exists(images_dir)) dir.create(images_dir, recursive = TRUE)
      dst_file <- file.path(images_dir, "error_type_composition.png")
      if (!file.exists(dst_file) || file.mtime(static_img) > file.mtime(dst_file)) {
        file.copy(static_img, dst_file, overwrite = TRUE)
      }
      # Change to output directory so relative path works (same pattern as other images)
      ...
      on.exit(setwd(old_wd), add = TRUE)
      setwd(output_dir)
      knitr::include_graphics("images/error_type_composition.png", dpi = 200)
    } else {
      cat("\n⚠ **Note:** Error type composition image not found at:", static_img, "\n")
    }
  }
```
...
```{r endpoint-heatmap-static}
#| fig-cap: "Endpoint Density: Static condition. White dashed lines mark target center (0,0)."
if (has_highcharter && !is.null(df_endpoint_int) && !is.null(endpoint_axis_info)) {
  ...
  if (nrow(df_endpoint_static_data) > 0) {
    ...
    static_img <- file.path(getwd(), "assets", "case_study", "portfolio", "heatmap_endpoint_density.png")
    if (file.exists(static_img)) {
      output_dir <- knitr::opts_knit$get("output.dir")
      if (is.null(output_dir)) {
        output_dir <- file.path(getwd(), "docs", "projects")
      ...
      images_dir <- file.path(output_dir, "images")
      if (!dir.exists(images_dir)) dir.create(images_dir, recursive = TRUE)
      dst_file <- file.path(images_dir, "heatmap_endpoint_density.png")
      if (!file.exists(dst_file) || file.mtime(static_img) > file.mtime(dst_file)) {
        file.copy(static_img, dst_file, overwrite = TRUE)
      }
      old_wd <- getwd()
      on.exit(setwd(old_wd), add = TRUE)
      setwd(output_dir)
      knitr::include_graphics("images/heatmap_endpoint_density.png", dpi = 200)
    }
  }
} else {
  # Fallback to static image
...
```{r endpoint-heatmap-adaptive}
#| fig-cap: "Endpoint Density: Adaptive condition"
if (has_highcharter && !is.null(df_endpoint_int) && !is.null(endpoint_axis_info)) {
...
Static to see if declutter affects endpoint clustering patterns.
...
```{r spatial-heatmap-hand}
#| fig-cap: "Spatial Error Rate: Hand modality"
if (has_highcharter && !is.null(df_spatial_int) && nrow(df_spatial_int) > 0) {
  ...
  if (nrow(df_hand_spatial) > 0) {
    ...
    # Fallback to static image
    static_img <- file.path(getwd(), "assets", "case_study", "portfolio", "heatmap_spatial_errors.png")
    if (file.exists(static_img)) {
      output_dir <- knitr::opts_knit$get("output.dir")
      if (is.null(output_dir)) {
        output_dir <- file.path(getwd(), "docs", "projects")
      ...
      images_dir <- file.path(output_dir, "images")
      if (!dir.exists(images_dir)) dir.create(images_dir, recursive = TRUE)
      dst_file <- file.path(images_dir, "heatmap_spatial_errors.png")
      if (!file.exists(dst_file) || file.mtime(static_img) > file.mtime(dst_file)) {
        file.copy(static_img, dst_file, overwrite = TRUE)
      }
      old_wd <- getwd()
      on.exit(setwd(old_wd), add = TRUE)
      setwd(output_dir)
      knitr::include_graphics("images/heatmap_spatial_errors.png", dpi = 200)
    }
  }
} else {
  # Fallback to static image
...
```{r spatial-heatmap-gaze}
#| fig-cap: "Spatial Error Rate: Gaze modality"
if (has_highcharter && !is.null(df_spatial_int) && nrow(df_spatial_int) > 0) {
...
**What to look for:** Warmer colors indicate higher error rates. Compare Hand vs Gaze to see if
...
and mechanisms need tuning. Not all "adaptive" changes
...
src_file <- NULL
possible_sources <- c(
  file.path(getwd(), "assets", "case_study", "portfolio", "psychophysics.png"),
  file.path(getwd(), "assets", "case_study", "psychophysics.png")
)
for (src in possible_sources) {
  if (file.exists(src)) {
    src_file <- src
...
if (is.null(src_file)) {
  stop("Psychophysics image not found. Checked:\n", paste("  -", possible_sources, collapse = "\n"))
}
...
```{r qc-summary}
if (file.exists("assets/case_study/qc_exclusions_summary.csv")) {
  qc <- read_csv("assets/case_study/qc_exclusions_summary.csv", show_col_types = FALSE)
...
```{r lba-results-table}
# Load LBA parameters - handle missing files gracefully
lba_params_file <- file.path(getwd(), "outputs", "LBA", "lba_parameters.json")
lba_summary_file <- file.path(getwd(), "outputs", "LBA", "lba_parameters_summary.csv")

if (!file.exists(lba_params_file) || !file.exists(lba_summary_file)) {
  cat("\n\n⚠ **Note:** LBA parameter files not found. Skipping LBA results table.\n")
  cat("Expected files:\n")
  cat("  -", lba_params_file, ifelse(file.exists(lba_params_file), "✓", "✗"), "\n")
  cat("  -", lba_summary_file, ifelse(file.exists(lba_summary_file), "✓", "✗"), "\n\n")
  # Create empty table structure to prevent errors
  lba_table <- tibble(
    Condition = character(0),
    `Non-Decision Time (t0)` = character(0),
    `Base Drift Rate` = character(0),
    `ID Slope` = character(0),
    `Threshold Intercept` = character(0),
    `Pressure Slope` = character(0)
  )
} else {
  tryCatch({
    lba_params <- jsonlite::fromJSON(lba_params_file)
    lba_summary <- read_csv(lba_summary_file, show_col_types = FALSE)
  ...
  tibble(
    ...
    cat("\n\n⚠ **Error loading LBA parameters:**", e$message, "\n\n")
...
# Only render table if we have data
if (exists("lba_table") && nrow(lba_table) > 0) {
  if (has_gt) {
    lba_table %>%
      gt() %>%
...
if (!file.exists(lba_summary_file)) {
  cat("\n\n⚠ **Note:** LBA convergence summary file not found. Skipping convergence diagnostics.\n")
  cat("Expected file:", lba_summary_file, "\n\n")
  # Create empty table to prevent errors
  lba_summary_display <- tibble(
    Parameter = character(0),
    Mean = character(0),
    `95% CI Lower` = character(0),
    `95% CI Upper` = character(0),
    `R-hat` = character(0),
    `ESS Bulk` = character(0)
  )
} else {

  lba_summary_raw <- read_csv(lba_summary_file, show_col_types = FALSE,
...
```{=html}
<div class="callout-note" style="padding:1rem; background:var(--color-bg-subtle);
     border-left:4px solid var(--color-accent); border-radius:0 8px 8px 0;
     margin:1rem 0; font-size:0.88rem;">
  <strong>Trace plots:</strong> Convergence was verified (R-hat &lt; 1.01,
  ESS &gt; 2,000 for all parameters — see table above). Trace plot image
  will be added in the final manuscript version.
</div>
```
...
**Note:** A detailed paper on the LBA analysis methodology, parameter interpretation, and theoretical implications is available as a preprint: Dastgheib, M., & Pourmahdian, F. (2026). *The Midas Touch in Gaze vs. Hand Pointing: Modality-Specific Failure Modes and Implications for XR Interfaces*. arXiv:2603.15991 [cs.HC]. <https://doi.org/10.48550/arXiv.2603.15991>
:::
