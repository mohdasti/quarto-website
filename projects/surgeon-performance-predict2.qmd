...
```{r}
#| echo: false
# Tiny demo: an inverted-U curve (intuition only)
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  ggplot(data.frame(arousal = x, performance = perf),
    ...
    geom_line() +
    labs(title = "Inverted-U intuition", y = "Performance (arb.)") +
    theme_minimal()
}
```

-   **Maps to Dashboard.** Wider `optimal_band` → fewer alerts but slower coaching; narrower band → tighter coaching with risk of nuisance alarms. Defaults exported from the Training Lab.
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  # ---- Parameters (mirror Training Lab semantics) ----
  sensitivity   <- 0.60     # 0..1  (liberal ↔ conservative)
  coupling      <- 0.70     # 0..1  (how tightly HL & Lapse move together)
  base_hl       <-  1.20    # baseline HL criterion (evidence axis units)
  base_lapse    <- -1.20    # baseline Lapse criterion (opposite side)
  move_range    <-  0.80    # total range each criterion can travel

  # Criteria move in opposite directions as sensitivity increases
  crit_hl    <- base_hl   - sensitivity * move_range
  crit_lapse <- base_lapse + sensitivity * move_range * coupling

  # Toy evidence distributions (illustrative SDT picture)
  set.seed(1)
  n    <- 1200
  x_n  <- rnorm(n, 0, 1)      # "normal/optimal" evidence
  x_hl <- rnorm(n, 2, 1)      # "high-load" evidence
  df   <- rbind(
    data.frame(x = x_n,  class = "Optimal/Normal"),
    data.frame(x = x_hl, class = "High-Load")
  )

  ggplot(df, aes(x, fill = class)) +
    geom_histogram(aes(y=..density..), bins = 60, alpha = 0.45, position = "identity") +
    geom_density(alpha = .4) +
    geom_vline(xintercept = crit_hl,   linetype = 2) +
    geom_vline(xintercept = crit_lapse,linetype = 3) +
    annotate("text", x = crit_hl + .15,   y = .35, label = "HL criterion →", size = 4) +
    annotate("text", x = crit_lapse - .15,y = .30, label = "← Lapse criterion", size = 4) +
    scale_fill_manual(values = c("Optimal/Normal" = "#0072B5FF", "High-Load" = "#BC3C29FF")) +
    labs(title = "Dual-Criterion (SDT): both thresholds move with one knob",
         subtitle = sprintf("sensitivity=%.2f, coupling=%.2f", sensitivity, coupling),
         x = "Evidence", y = "Density") +
    theme_minimal()
}

```

-   **Maps to Dashboard.** Sets the `high_load_prob` slider + post-hoc fusion guardrails (e.g., require **TEPR rise** or **HRV drop** to confirm).

-   **References.** Macmillan & Creelman (Signal Detection Theory); NASA-TLX background (Hart & Staveland).

------------------------------------------------------------------------

### Time-on-Task (Fatigue) <span class="help-icon" data-bs-toggle="tooltip" title="Hold steady until onset, then relax with half-life; microbreak gives a brief reset with exponential decay."><i class="bi bi-info-circle"></i></span>
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  # --- Parameters (use the same names you present in the text) ---
  c_start              <- 0.70   # starting decision threshold
  c_floor              <- 0.50   # most relaxed threshold allowed
  onset_min            <- 30     # fatigue onset time (min)
  fatigue_half_life_min<- 21     # how fast threshold relaxes (half-life, min)
  microbreak_min       <- 60     # minute of the microbreak (set NA for none)
  microbreak_reset     <- 0.12   # how strongly the break recenters the threshold
  microbreak_decay_min <- 2      # how quickly that reset fades (min)

  # --- Helper (exponential relaxation + smooth microbreak recovery) ---
  fatigue_threshold <- function(t) {
    tau  <- fatigue_half_life_min / log(2)   # convert half-life to time constant
    # baseline: hold at c_start until onset, then exponential toward c_floor
    base <- ifelse(t < onset_min,
                   c_start,
                   c_floor + (c_start - c_floor) * exp(-(t - onset_min) / tau))

    # optional microbreak bump with exponential decay
    boost <- if (!is.na(microbreak_min)) {
      microbreak_reset * exp(-pmax(0, t - microbreak_min) / microbreak_decay_min)
    } else 0
...
cat(
  ...
  "- **Pre-onset (t < ", onset_min, " min):** threshold stays at **", fmt(c_start), "** (no fatigue yet).\n",
  "- **Post-onset:** threshold relaxes **exponentially** toward the **floor** **", fmt(c_floor),
    "** → more tolerant of High-Load detections as vigilance degrades.\n",
  if (!is.na(microbreak_min))
  ...
  "- **Half-life marker:** at **t = onset + half-life = ", onset_min, " + ", fatigue_half_life_min,
    ...
    "** and **", fmt(c_floor), "**.\n",
  "- Visual cues: **solid line** = start level (", fmt(c_start),
